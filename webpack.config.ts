...
<<<<<<< HEAD
  ...
  const is_force_include_entry = (file: string) => {
    ...
    return normalized.startsWith('src/') && normalized.endsWith(force_include_suffix);
  };
...
<<<<<<< HEAD
      ...
      for (const [index, result] of results.entries()) {
        const is_force_include_result = is_force_include_entry(result);
        const result_dirname = path.dirname(result);
        const common = common_path(result_dirname, file_dirname);
        if (!is_force_include_file && common === result_dirname) {
          return;
        }
